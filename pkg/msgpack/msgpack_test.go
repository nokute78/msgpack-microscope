...
func TestDecode(t *testing.T) {
	...
	cases := []testcase{
		{"p fixint", []byte{0x01}, &MPObject{DataStr: "1", FormatName: "positive fixint"}},
		{"n fixint", []byte{0xff}, &MPObject{DataStr: "-1", FormatName: "negative fixint"}},
		{"nil", []byte{0xc0}, &MPObject{DataStr: "nil", FormatName: "nil"}},
		{"never used", []byte{0xc1}, &MPObject{DataStr: "(never used)", FormatName: "(never used)"}},
		{"true", []byte{0xc3}, &MPObject{DataStr: "true", FormatName: "true"}},
		{"false", []byte{0xc2}, &MPObject{DataStr: "false", FormatName: "false"}},
		{"float32", []byte{0xca, 0x80, 0x00, 0x00, 0x00}, &MPObject{DataStr: "-0.000000", FormatName: "float 32"}},
		{"float64", []byte{0xcb, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, &MPObject{DataStr: "-0.000000", FormatName: "float 64"}},
		{"uint8", []byte{0xcc, 0xff}, &MPObject{DataStr: "255", FormatName: "uint 8"}},
		{"uint16", []byte{0xcd, 0xff, 0x00}, &MPObject{DataStr: "65280", FormatName: "uint 16"}},
		{"uint32", []byte{0xce, 0xff, 0x00, 0xff, 0x00}, &MPObject{DataStr: "4278255360", FormatName: "uint 32"}},
		{"uint64", []byte{0xcf, 0xff, 0x00, 0xff, 0x00, 0xff, 0x00, 0xff, 0x00}, &MPObject{DataStr: "18374966859414961920", FormatName: "uint 64"}},
		{"int8", []byte{0xd0, 0xff}, &MPObject{DataStr: "-1", FormatName: "int 8"}},
		{"int16", []byte{0xd1, 0xff, 0x00}, &MPObject{DataStr: "-256", FormatName: "int 16"}},
		{"int32", []byte{0xd2, 0xff, 0x00, 0xff, 0x00}, &MPObject{DataStr: "-16711936", FormatName: "int 32"}},
		{"int64", []byte{0xd3, 0xff, 0x00, 0xff, 0x00, 0xff, 0x00, 0xff, 0x00}, &MPObject{DataStr: "-71777214294589696", FormatName: "int 64"}},
		{"str8", []byte{0xd9, 0x0f, 0xe3, 0x81, 0x93, 0xe3, 0x82, 0x93, 0xe3, 0x81, 0xab, 0xe3, 0x81, 0xa1, 0xe3, 0x81, 0xaf}, &MPObject{DataStr: "こんにちは", FormatName: "str 8"}},
		{"bin8", []byte{0xc4, 0x04, 0xde, 0xad, 0xbe, 0xef}, &MPObject{DataStr: "0xdeadbeef", FormatName: "bin 8"}},
		{"fixstr len31", []byte{0xbf, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31}, &MPObject{DataStr: "1234567890123456789012345678901", FormatName: "fixstr"}},
		{"fixarray len2", []byte{0x92, 0x00, 0x01}, &MPObject{DataStr: "(fixarray)", FormatName: "fixarray"}},
		{"fixmap len2", []byte{0x82, 0xa1, 0x30, 0x00, 0xa1, 0x31, 0x01}, &MPObject{DataStr: "(fixmap)", FormatName: "fixmap"}},
		{"array16", []byte{0xdc, 0x00, 0x0f, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00}, &MPObject{DataStr: "(array 16)", FormatName: "array 16"}},
		{"fixext1", []byte{0xd4, 0x01, 0xff}, &MPObject{DataStr: "0xff", FormatName: "fixext 1", ExtType: 1}},
		{"fixext2", []byte{0xd5, 0x01, 0xfe, 0xed}, &MPObject{DataStr: "0xfeed", FormatName: "fixext 2", ExtType: 1}},
		{"fixext4", []byte{0xd6, 0x01, 0xde, 0xad, 0xbe, 0xef}, &MPObject{DataStr: "0xdeadbeef", FormatName: "fixext 4", ExtType: 1}},
		{"fixext8", []byte{0xd7, 0x01, 0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe, 0xef}, &MPObject{DataStr: "0xdeadbeefdeadbeef", FormatName: "fixext 8", ExtType: 1}},
		{"ext8", []byte{0xc7, 0x04, 0x01, 0xde, 0xad, 0xbe, 0xef}, &MPObject{DataStr: "0xdeadbeef", FormatName: "ext 8", ExtType: 1}},
	}

	/* str16 */
	strcase := testcase{casename: "str16", obj: &MPObject{DataStr: strings.Repeat("こんにちは", 20), FormatName: "str 16"}}
	...
	/* str32 */
	strcase = testcase{casename: "str32", obj: &MPObject{DataStr: strings.Repeat("こんにちは", 4370), FormatName: "str 32"}}
	...
	strcase = testcase{casename: "bin16", obj: &MPObject{DataStr: fmt.Sprintf("0x%x", bytes.Repeat(deadbeef, 64)), FormatName: "bin 16"}}
	...
	/* bin32 */
	strcase = testcase{casename: "bin32", obj: &MPObject{DataStr: fmt.Sprintf("0x%x", bytes.Repeat(deadbeef, 16384)), FormatName: "bin 32"}}
	...
	/* ext16 */
	strcase = testcase{casename: "ext16", obj: &MPObject{DataStr: fmt.Sprintf("0x%x", bytes.Repeat(deadbeef, 64)), FormatName: "ext 16", ExtType: 1}}
	...
	/* ext32 */
	strcase = testcase{casename: "ext32", obj: &MPObject{DataStr: fmt.Sprintf("0x%x", bytes.Repeat(deadbeef, 16384)), FormatName: "ext 32", ExtType: 1}}
	...
	/* array32 */
	strcase = testcase{casename: "array32", obj: &MPObject{DataStr: "(array 32)", FormatName: "array 32"}}
	...
	/* map16 */
	strcase = testcase{casename: "map16", obj: &MPObject{DataStr: "(map 16)", FormatName: "map 16"}}
	...
	/* map32 */
	strcase = testcase{casename: "map32", obj: &MPObject{DataStr: "(map 32)", FormatName: "map 32"}}
	...
	for _, v := range cases {
		...
		if ret.FormatName != v.obj.FormatName {
			t.Errorf("%s: FormatName mismatch: %s, expect %s", v.casename, ret.FormatName, v.obj.FormatName)
		}
